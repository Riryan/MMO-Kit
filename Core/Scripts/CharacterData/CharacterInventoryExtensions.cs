...
namespace MultiplayerARPG
{
    public static class CharacterInventoryExtensions
    {
        ...
        public static bool DismantleItem(this IPlayerCharacterData character, int index, int amount, out UITextKeys gameMessage)
        {
            ...
            // Prevent duplicate dismantle for the same player/action within a short time window
            if (!CurrencyTransactionGuard.BeginTransaction(character.Id, "DismantleItem"))
            {
                gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
                return false;
            }
            try
            {
                ...
                // Clone inventory for simulation before actually removing anything
                List<CharacterItem> simulatingNonEquipItems = character.NonEquipItems.Clone();
                // Check if dismantle is allowed and get results
                if (!character.VerifyDismantleItem(index, amount, simulatingNonEquipItems,
                                           out gameMessage,
                                           out dismentleItem,
                                           out returningGold,
                                           out returningItems,
                                           out returningCurrencies))
                return false;
                // Package the dismantled item in a list for logging
                ...
                // Apply dismantle results
                character.Gold = character.Gold.Increase(returningGold);    // ADD GOLD
                character.DecreaseItemsByIndex(index, amount, true);        // Remove dismantled item
                character.IncreaseItems(returningItems);                    // Add material/item rewards
                character.IncreaseCurrencies(returningCurrencies);          // Add currency rewards (could be gold too)
                character.FillEmptySlots();
                // Log the dismantle operation server-side
            ...
            finally
            {
                ...
                CurrencyTransactionGuard.EndTransaction(character.Id, "DismantleItem");
            }
#else
                gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
                 return false;
...
public static bool DismantleItems(this IPlayerCharacterData character, int[] selectedIndexes, out UITextKeys gameMessage)
{
#if UNITY_EDITOR || UNITY_SERVER
    // Prevent duplicate dismantle for the same player/action within a short time window
    if (!CurrencyTransactionGuard.BeginTransaction(character.Id, "DismantleItems"))
    {
        gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
        return false;
    }
    ...
    try
    {
        ...
        for (int i = indexes.Count - 1; i >= 0; --i)
        {
            ...
            if (!character.VerifyDismantleItem(tempIndex, tempAmount, simulatingNonEquipItems,
                                               out gameMessage,
                                               out tempDismentleItem,
                                               out tempReturningGold,
                                               out tempReturningItems,
                                               out tempReturningCurrencies))
                return false;
        ...
        // Apply rewards and remove items
    ...
    {
        // Always release the transaction lock
        CurrencyTransactionGuard.EndTransaction(character.Id, "DismantleItems");
    }
#else
    gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
    return false;
...
{
#if UNITY_EDITOR || UNITY_SERVER
    // Prevent duplicate refine for the same player/action within a short time window
    if (!CurrencyTransactionGuard.BeginTransaction(character.Id, "RefineItem"))
    {
        gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
        return false;
    }
    ...
    {
        // Always release the transaction lock
        CurrencyTransactionGuard.EndTransaction(character.Id, "RefineItem");
    }
#else
    gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
    return false;
...
{
#if UNITY_EDITOR || UNITY_SERVER
    // Prevent duplicate repair requests for the same player/action within a short time window
    if (!CurrencyTransactionGuard.BeginTransaction(character.Id, "RepairItem"))
    {
        gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
        return false;
    }
    ...
    {
        // Always release the transaction lock
        CurrencyTransactionGuard.EndTransaction(character.Id, "RepairItem");
    }
#else
    gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
    return false;
...
{
#if UNITY_EDITOR || UNITY_SERVER
    // Prevent duplicate sell requests for the same player/action within a short time window
    if (!CurrencyTransactionGuard.BeginTransaction(character.Id, "SellItem"))
    {
        gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
        return false;
    }
    ...
    {
        // Always release the transaction lock
        CurrencyTransactionGuard.EndTransaction(character.Id, "SellItem");
    }
#else
    gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
    return false;
...
{
#if UNITY_EDITOR || UNITY_SERVER
    // Prevent duplicate bulk-sell requests for the same player/action within a short time window
    if (!CurrencyTransactionGuard.BeginTransaction(character.Id, "SellItems"))
    {
        gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
        return false;
    }
    ...
    try
    {
        ...
        for (int i = indexes.Count - 1; i >= 0; --i)
        {
            ...
            // Use guarded single-item sell to benefit from its checks
    ...
    {
        // Always release the transaction lock
        CurrencyTransactionGuard.EndTransaction(character.Id, "SellItems");
    }
#else
    gameMessage = UITextKeys.UI_ERROR_TRY_AGAIN_LATER;
    return false;
